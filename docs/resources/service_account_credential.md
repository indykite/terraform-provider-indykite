...
page_title: "indykite_service_account_credential Resource - IndyKite"
subcategory: ""
description: |-
  Service Account Credential is a JSON configuration file that contains a secret key or token for authenticating to IndyKite APIs.
---

# indykite_service_account_credential (Resource)
...
- `service_account_id` (String) Identifier of Service Account to which the credential will be registered.

### Optional

- `display_name` (String) Optional human readable name of the credential.
- `expire_time` (String) Optional date-time when credentials are going to expire in RFC3339 format.
...
- `kid` (String) Key identifier of the credential.
- `service_account_config` (String, Sensitive) JSON configuration of the created credential. This is only available after creation.
